...
public class DynamicAllocationStreamTests
{
    ...
    public void Sparse_gap_reads_as_zero_after_reopen()
    {
        ...
        backing.Position = 0;
        using DynamicAllocationStream opened = DynamicAllocationStream.Open(backing, TestOptions);
    ...
    public void Forced_read_only_mode_allows_reads_and_rejects_mutation()
    {
        ...
        var readOnlyOptions = new DynamicAllocationStreamOptions(
            leaveOpen: true,
            readOnly: true,
            freeBlockQueueCapacity: 0,
            freeBlockQueueLowWatermark: 0);
        using DynamicAllocationStream opened = DynamicAllocationStream.Open(backing, readOnlyOptions);
    ...
    public void Active_journal_is_replayed_after_interrupted_home_write()
    {
        ...
        storage.Position = 0;
        using DynamicAllocationStream recovered = DynamicAllocationStream.Open(storage, TestOptions);
    ...
    public void Active_journal_can_be_replayed_into_read_only_overlay()
    {
        ...
        using DynamicAllocationStream recovered = DynamicAllocationStream.Open(readOnlyBacking, TestOptions);
    ...
    public async Task Async_write_flush_open_and_read_round_trip()
    {
        ...
        await using DynamicAllocationStream opened = await DynamicAllocationStream.OpenAsync(
            backing,
            TestOptions,
            TestContext.Current.CancellationToken);
        opened.Position = BlockSize - 2;
    ...
    public void Invalid_magic_and_roots_are_rejected()
    {
        ...
        Assert.Throws<DynamicAllocationStreamCorruptionException>(() => DynamicAllocationStream.Open(backing, TestOptions));
    }
