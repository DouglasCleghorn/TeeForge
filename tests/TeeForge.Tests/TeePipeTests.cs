...
public class TeePipeTests
{
    [Fact]
    public void Constructor_requires_positive_reader_count()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new TeePipe(0));
        Assert.Throws<ArgumentOutOfRangeException>(() => new TeePipe(-1));
    }

    [Fact]
    public async Task Every_reader_observes_the_complete_sequence_independently()
    {
        var pipe = new TeePipe(3);
    ...
    {
        var options = new TeePipeOptions(pauseWriterThreshold: 4, resumeWriterThreshold: 2, useSynchronizationContext: false);
        var pipe = new TeePipe(2, options);
    ...
    {
        var options = new TeePipeOptions(readerFailureBehavior: TeePipeReaderFailureBehavior.CompletePipe);
        var pipe = new TeePipe(2, options);
    ...
    public async Task CompletePipe_uses_first_reader_fault_and_retains_each_completion_exception()
    {
        var pipe = new TeePipe(
            3,
            new TeePipeOptions(readerFailureBehavior: TeePipeReaderFailureBehavior.CompletePipe));
    ...
    private static void Complete(TeePipe pipe)
    {
        pipe.Writer.Complete();
        foreach (PipeReader reader in pipe.Readers)
        {
