...
public class DifferencingDiskImageTests
{
    private const int BlockSize = 64 * 1024;
    private const long Capacity = 4L * BlockSize;
    private static readonly SparseDiskImageOptions DynamicOptions = new(
        leaveOpen: true,
        freeBlockQueueCapacity: 0,
        freeBlockQueueLowWatermark: 0);
    private static readonly DifferencingDiskImageOptions DifferenceOptions = new(
        leaveBaseOpen: true,
    ...
    public void Inherited_read_matches_base_and_partial_write_changes_only_child_grain()
    {
        ...
        using SparseDiskImage baseDisk = CreateBase(baseStorage);
    ...
    public async Task Locator_is_validated_by_library_and_preserves_stream_position()
    {
        ...
        DifferencingDiskImageLocator locator = await DifferencingDiskImage.ReadLocatorAsync(differenceStorage);
        ...
        Assert.Throws<IOException>(() => DifferencingDiskImage.ReadLocator(corruptStorage));
    }
    ...
    public void Partial_state_and_erased_tail_survive_reopen()
    {
        ...
        using (DifferencingDiskImage child = DifferencingDiskImage.Create(baseDisk, differenceStorage, DifferenceOptions))
        {
            childId = child.Id;
        ...
        using DifferencingDiskImage reopened = DifferencingDiskImage.Open(baseDisk, differenceStorage, DifferenceOptions);
    ...
    public async Task Explicit_offset_async_io_does_not_change_position()
    {
        ...
        baseDisk.Flush();
        using var differenceStorage = new MemoryStream();
        await using DifferencingDiskImage child = DifferencingDiskImage.Create(baseDisk, differenceStorage, DifferenceOptions);
        child.Position = 77;
    ...
    public void Open_rejects_changed_base_data_identity()
    {
        ...
        using SparseDiskImage reopenedBase = SparseDiskImage.Open(baseStorage, DynamicOptions);
        reopenedBase.WriteAt([2], 0);
        reopenedBase.Flush();

        Assert.Throws<DifferencingDiskImageBaseMismatchException>(() =>
            DifferencingDiskImage.Open(reopenedBase, differenceStorage, DifferenceOptions));
    }
    ...
    public void Notify_on_create_registers_immediate_child_without_changing_base_data_identity()
    {
        ...
        using var differenceStorage = new MemoryStream();
        var options = new DifferencingDiskImageOptions(
            leaveBaseOpen: true,
            leaveDifferenceOpen: true,
            notifyBaseOnCreate: true);

        using DifferencingDiskImage child = DifferencingDiskImage.Create(baseDisk, differenceStorage, options);
    ...
    public void Differencing_chain_reads_parent_overlay_and_writes_only_leaf()
    {
        ...
        using DifferencingDiskImage child = DifferencingDiskImage.Create(baseDisk, childStorage, DifferenceOptions);
        child.WriteAt([7], 100);
        child.Flush();
        Guid childDataWriteId = child.DataWriteId;
        using var leafStorage = new MemoryStream();
        using DifferencingDiskImage leaf = DifferencingDiskImage.Create(child, leafStorage, DifferenceOptions);
    ...
    public void Dynamic_capacity_and_registry_persist_without_changing_data_identity()
    {
        ...
        Guid dataWriteId;
        using (SparseDiskImage disk = CreateBase(storage))
        {
        ...
        storage.Position = 0;
        using SparseDiskImage reopened = SparseDiskImage.Open(storage, DynamicOptions);
    ...
    public void Compaction_remains_openable_at_every_injected_write_failure()
    {
        ...
        using (DifferencingDiskImage child = DifferencingDiskImage.Open(baseDisk, counter, DifferenceOptions))
        {
            child.Compact();
        ...
        for (int failAt = 1; failAt <= compactionWriteCount; failAt++)
        {
            ...
            using (DifferencingDiskImage child = DifferencingDiskImage.Open(baseDisk, faulting, DifferenceOptions))
            {
                Assert.Throws<IOException>(() => child.Compact());
            }

            inner.Position = 0;
            using DifferencingDiskImage reopened = DifferencingDiskImage.Open(baseDisk, inner, DifferenceOptions);
            Assert.Equal(expected, ReadAt(reopened, 0, expected.Length));
        }
    }

    private static SparseDiskImage CreateBase(MemoryStream storage) =>
        SparseDiskImage.Create(storage, Capacity, BlockSize, DynamicOptions);
