...
public class ErasureImageTests
{
    ...
    public async Task Self_describing_set_round_trips_sequential_and_random_io()
    {
        ...
        var options = new ErasureImageOptions(leaveOpen: true, readAheadBlockCount: 2);
        ...
        await using ErasureImage reopened = ErasureImage.Open(reordered, options);
    ...
    {
        MemoryStream[] members = CreateMembers(DataCount + ParityCount);
        var options = new ErasureImageOptions(leaveOpen: true);
        using ErasureImage stream = ErasureImage.Create(
            members, DataCount, ParityCount, Length, BlockSize, options);

        ErasureImageHeader[] headers = members.Select(ErasureImageHeaderParser.Read).ToArray();
        ErasureImageHeader parsedBytes = ErasureImageHeaderParser.Parse(members[1].ToArray().AsSpan(0, 4096));
        ...
        ErasureImageHeader recovered = ErasureImageHeaderParser.Read(members[0]);
    ...
    public async Task Forward_only_members_encode_a_partial_final_codeword()
    {
        ...
        var options = new ErasureImageOptions(leaveOpen: true, maximumCacheBytes: BlockSize);
        await using (ErasureImage stream = ErasureImage.Create(
        ...
        await using ErasureImage reopened = ErasureImage.Open(storage, options);
    ...
    public async Task Degraded_open_is_read_only_and_reconstructs_a_missing_data_member()
    {
        MemoryStream[] members = CreateMembers(DataCount + ParityCount);
        byte[] payload = CreatePayload(checked((int)Length));
        var createOptions = new ErasureImageOptions(leaveOpen: true);
        await using (ErasureImage stream = ErasureImage.Create(
        ...
        Assert.Throws<InvalidDataException>(() => ErasureImage.Open(survivors, createOptions));

        var degradedOptions = new ErasureImageOptions(requireAllMembers: false, leaveOpen: true);
        await using ErasureImage degraded = ErasureImage.Open(survivors, degradedOptions);
    ...
    public async Task Missing_parity_image_can_be_replaced_and_reopened()
    {
        ...
        await using ErasureImage reopened = ErasureImage.Open(restored, options);
        var actual = new byte[payload.Length];
        await reopened.ReadAtAsync(actual, 0);
        Assert.Equal(payload, actual);
    }
    ...
    public async Task Parity_can_be_added_and_trailing_parity_can_be_removed()
    {
        ...
        var degradedOptions = new ErasureImageOptions(requireAllMembers: false, leaveOpen: true);
        await using (ErasureImage degraded = ErasureImage.Open(addedParitySurvivors, degradedOptions))
        {
        ...
        await using (ErasureImage expanded = ErasureImage.Open(expandedMembers, options))
        {
            IReadOnlyList<Stream> removed = await expanded.ReduceParityAsync(1);
            Assert.Same(added, Assert.Single(removed));
        }

        await using ErasureImage reopened = ErasureImage.Open(members, options);
        var actual = new byte[payload.Length];
        await reopened.ReadAtAsync(actual, 0);
        Assert.Equal(payload, actual);
    }
