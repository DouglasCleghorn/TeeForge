...
// Adapted for TeeForge from System.IO.BufferedStream at dotnet/runtime commit
...
/// </summary>
public class TeeBufferedStream : Stream, ITeeRandomAccessStream, ITeeRangeReadSource
{
    ...
    private TeeStream? _stream;                                  // Mirrored streams. Close sets _stream to null.
    ...
    /// <summary>Initializes a buffered tee over the supplied destination streams.</summary>
    /// <param name="destinations">The streams that receive each operation.</param>
    public TeeBufferedStream(params Stream[] destinations)
    ...
    /// <param name="destinations">The streams that receive each operation.</param>
    public TeeBufferedStream(int bufferSize, params Stream[] destinations)
        : this(
            (IEnumerable<Stream>)destinations,
            new TeeBufferedStreamOptions(bufferSize: bufferSize))
    {
    }

    /// <summary>Initializes a buffered tee with explicit buffered behavior.</summary>
    /// <param name="options">The buffering and tee behavior.</param>
    /// <param name="destinations">The streams that receive each operation.</param>
    public TeeBufferedStream(TeeBufferedStreamOptions options, params Stream[] destinations)
        : this((IEnumerable<Stream>)destinations, options)
    {
    }

    /// <summary>Initializes a buffered tee from a destination sequence.</summary>
    /// <param name="destinations">The streams that receive each operation.</param>
    /// <param name="options">The buffering and tee behavior, or <see langword="null"/> for defaults.</param>
    public TeeBufferedStream(
        IEnumerable<Stream> destinations,
        TeeBufferedStreamOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(destinations);

        TeeBufferedStreamOptions effectiveOptions = options ?? TeeBufferedStreamOptions.Default;
        _stream = new CachedCapabilityTeeStream(destinations, effectiveOptions);
        _bufferSize = effectiveOptions.BufferSize;

        // Allocate _buffer on its first use - it will not be used if all reads
        // & writes are greater than or equal to buffer size.

        ObjectDisposedException.ThrowIf(!_stream.CanRead && !_stream.CanWrite, _stream);
    }
    ...
    private SemaphoreSlim EnsureAsyncActiveSemaphoreInitialized() =>
        LazyInitializer.EnsureInitialized(
            ...
            static () => new SemaphoreSlim(initialCount: 1, maxCount: 1));

    private Task<int> GetCachedCompletedReadTask(int result)
    {
        Task<int>? task = _lastSyncCompletedReadTask;
        if (task is null || task.Result != result)
        {
            task = Task.FromResult(result);
            _lastSyncCompletedReadTask = task;
        }
    ...
    /// <summary>Gets the unbuffered tee receiving emitted operations.</summary>
    public TeeStream UnderlyingStream
    {
        get
        {
    ...
    /// <summary>Gets the size in bytes of the shared read/write buffer.</summary>
    ...
    /// <inheritdoc/>
    public bool CanReadAt => _stream?.CanReadAt == true;

    /// <inheritdoc/>
    public bool CanWriteAt => _stream?.CanWriteAt == true;
    ...
    public int ReadAt(Span<byte> buffer, long offset)
    {
        ...
        try
        {
            ...
            return _stream!.ReadAt(buffer, offset);
        }
        finally
        {
            semaphore.Release();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return await _stream!.ReadAtAsync(buffer, offset, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            semaphore.Release();
    ...
    public void WriteAt(ReadOnlySpan<byte> buffer, long offset)
    {
        ...
        try
        {
            PrepareForRandomAccessWrite();
            _stream!.WriteAt(buffer, offset);
        }
        finally
        {
            semaphore.Release();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _stream!.WriteAtAsync(buffer, offset, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            semaphore.Release();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return await _stream!.OpenReadRangeAsync(offset, length, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            semaphore.Release();
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        finally
        {
            ...
            Interlocked.Exchange(ref _asyncActiveSemaphore, null)?.Dispose();
    ...
    private void ClearReadBufferBeforeWrite()
    {
        ...
        if (!_stream.CanSeek)
            throw new NotSupportedException(
                "Writing is not supported while unread buffered data cannot be returned to a non-seekable stream.");
    ...
    private void PrepareForRandomAccessWrite()
    {
        if (_writePos > 0)
        {
            FlushWrite();
    ...
    public override int Read(Span<byte> buffer)
    {
        ...
        if (buffer.Length >= _bufferSize)
        {
            ...
            return _stream.Read(buffer) + bytesFromBuffer;
        }
        else
        {
    ...
    public override Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        ...
        if (semaphoreLockTask.IsCompletedSuccessfully)
        {
            ...
            try
            {
                ...
                if (completeSynchronously)
                {
                    ...
                    return (error == null)
                                ? GetCachedCompletedReadTask(bytesFromBuffer)
        ...
        return ReadFromUnderlyingStreamAsync(
            ...
            bytesFromBuffer, sem, semaphoreLockTask, cancellationToken).AsTask();
    }
    ...
    public override ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        // Delegate to the async implementation.
        return WriteToUnderlyingStreamAsync(buffer, sem, semaphoreLockTask, cancellationToken);
    }
    ...
    private sealed class CachedCapabilityTeeStream : TeeStream
    {
        ...
        public CachedCapabilityTeeStream(IEnumerable<Stream> destinations, TeeStreamOptions? options)
            : base(destinations, options)
        {
            // BufferedStream checks capabilities on its hot paths. Snapshot the conventional
            // lifetime-stable capability values while retaining TeeStream's operation semantics.
            _canRead = base.CanRead;
            _canSeek = base.CanSeek;
            _canTimeout = base.CanTimeout;
            _canWrite = base.CanWrite;
        }
        ...
        public override bool CanTimeout => !IsDisposed && _canTimeout;
