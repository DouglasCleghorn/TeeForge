...
using TeeForge.Pipelines.Internal;
...
public class TeePipe
{
    ...
    private static readonly Action<object?> SignalWriterAwaitable = static state =>
        ((TeePipe)state!).WriterCancellationRequested();
    ...
    // The sole TeePipe lock. It protects all shared state transitions.
    private readonly Lock _sync = new();
    private readonly TeePipeOptions _options;
    ...
    /// <summary>Initializes a TeePipe with the default options.</summary>
    /// <param name="readerCount">The fixed positive number of broadcast readers.</param>
    public TeePipe(int readerCount)
        : this(readerCount, TeePipeOptions.Default)
    {
    }

    /// <summary>Initializes a TeePipe with explicit options.</summary>
    /// <param name="readerCount">The fixed positive number of broadcast readers.</param>
    /// <param name="options">The pipe options.</param>
    public TeePipe(int readerCount, TeePipeOptions options)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(readerCount);

        ArgumentNullException.ThrowIfNull(options);
        _options = options;
        _bufferSegmentPool = new BufferSegmentStack(TeePipeOptions.InitialSegmentPoolSize);
    ...
    public void Reset()
    {
        lock (_sync)
        {
            if (!_disposed)
            {
                throw new InvalidOperationException("The writer and every reader must complete before TeePipe can be reset.");
            }
    ...
    {
        if (_bufferSegmentPool.Count < TeePipeOptions.MaxSegmentPoolSize)
        {
    ...
    private void CompleteReader(ReaderState reader, Exception? exception)
    {
        ...
        lock (_sync)
        {
            ...
            if (exception is not null
                && _options.ReaderFailureBehavior == TeePipeReaderFailureBehavior.CompletePipe
        ...
        ValueTaskSourceOnCompletedFlags flags)
    {
        ...
        if (doubleCompletion)
        {
            CompleteWriter(new InvalidOperationException("Concurrent operations on one TeePipe reader are not supported."));
        }
        ...
        ValueTaskSourceOnCompletedFlags flags)
    {
        ...
        if (doubleCompletion)
        {
            ...
            {
                reader.Complete(new InvalidOperationException("Concurrent TeePipe writer operations are not supported."));
            }
        }
    ...
    private void ReclaimSegmentsUnsynchronized()
    {
        ...
        while (_bufferHead is not null)
        {
            ...
            _bufferHead = segment.NextSegment;
            segment.Reset();
            ReturnSegmentUnsynchronized(segment);
        }
    }
    ...
    {
        public ReaderState(TeePipe owner, int index)
        {
            Owner = owner;
            Index = index;
        }

        public TeePipe Owner { get; }
    ...
    {
        private readonly TeePipe _pipe;
        private readonly ReaderState _state;

        public DefaultPipeReader(TeePipe pipe, ReaderState state)
        {
            _pipe = pipe;
            _state = state;
            state.Endpoint = this;
...
#pragma warning restore CS0672
    ...
    private sealed class DefaultPipeWriter : PipeWriter, IValueTaskSource<FlushResult>
    {
        ...
        public DefaultPipeWriter(TeePipe pipe) => _pipe = pipe;
