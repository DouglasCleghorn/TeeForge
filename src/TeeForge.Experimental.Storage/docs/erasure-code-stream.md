...
`ErasureCodedVolume` presents a set of seekable member streams as one
...
var options = new ErasureCodedVolumeOptions(
    leaveOpen: true,
    journalSlotCount: 4,
    latencySampleRate: 64);

await using ErasureCodedVolume volume = await ErasureCodedVolume.CreateAsync(
    memberStreams,
    dataShardCount: 6,
    parityShardCount: 2,
