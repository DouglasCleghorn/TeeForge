...
`DifferencingDiskImageBaseMismatchException`. A TeeForge parent must also match the
...
`DifferencingDiskImage.ReadLocator` and `ReadLocatorAsync` validate the identifier
