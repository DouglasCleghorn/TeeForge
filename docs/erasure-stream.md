...
`ErasureStream` is the lightweight, stream-shaped erasure-coding layer. It maps
one fixed-length logical stream onto `k` systematic data members and `m` parity
members without a stripe journal. Use `ErasureCodeStream` when an interrupted
random overwrite must be recovered transactionally; use `ErasureStream` when
ordinary crash consistency is sufficient or the members are forward-only.
...
between `ErasureStream` and the journaled implementation.
...
`ErasureStreamHeaderParser.Parse` and `TryParse` inspect a caller-supplied 4 KiB
...
Set `ErasureStreamOptions.Format` to `Raw` to omit all metadata. Raw members
start with payload byte zero and must be reopened with `OpenRaw` plus externally
supplied geometry and member order.
